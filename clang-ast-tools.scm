    ...
    (syntax-case stx ()
      ((_ xname (sym . int) ...)
       (let* ((name (symbol->string (syntax->datum #'xname)))
         ...
         (with-syntax ((xname/sym->int (datum->syntax #'xname (string->symbol name/sym->int)))
           ...
           #'(begin
               (define xname/sym->int (xalist->hashq-table '((sym . int) ...)))
               (define xname/int->sym (xalist->hashq-table '((int . sym) ...))) ;; should this be hashv (eqv? for numbers)
